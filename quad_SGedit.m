...
%----New functions for Q1(b,c,d)

 GetJacobian; %subroutine to calculate Jacobian at each node
 GetNodalStress;
GetAvgStress;
...
%-----------------------------------------------ADD-------------------------------------------%
disp('  1) 1x1 Gaussian quadrature');
disp('  2) 1x2 Gaussian quadrature');
disp('  3) 2x2 Gaussian quadrature');
...
%-------------------------------------EDITED----------------------------------------------------%
disp(blanks(1));
FILE1 = "mesh_coarse.txt"; %input('Input Data File Name ','s');
LINP  = fopen(FILE1,'r');
...
%  --- Weight Factor is DIFFERENT  
   WEIGHTS_11 = [4];
   WEIGHTS_12 = [2,2];
   WEIGHTS_22 = [1,1,1,1];


%  --- Loop on Integration Points----------CHANGED LOOP-----------------%
   for IP = 1:NIP
...
%  --- Modify stiffness matrix after implementing weights
        for I = 1:8
            for J = 1:8
                if GP==1
...
% 
% 
%     J11 = -(1-eta)*nnode(elements(i,2),2)+(1-eta)*nnode(elements(i,3),2)+(1+eta)*nnode(elements(i,4),2)-(1+eta)*nnode(elements(i,5),2);
%     J12 = -(1-eta)*nnode(elements(i,2),3)+(1-eta)*nnode(elements(i,3),3)+(1+eta)*nnode(elements(i,4),3)-(1+eta)*nnode(elements(i,5),3);
%     J21 = -(1-zeta)*nnode(elements(i,2),2)-(1+zeta)*nnode(elements(i,3),2)+(1+zeta)*nnode(elements(i,4),2)+(1-zeta)*nnode(elements(i,5),2);
%     J22 = -(1-zeta)*nnode(elements(i,2),3)-(1+zeta)*nnode(elements(i,3),3)+(1+zeta)*nnode(elements(i,4),3)+(1-zeta)*nnode(elements(i,5),3);
%     Jacob = 0.25*[J11 J12;J21 J22];
% 
%     JL(elements(i,j))=det(Jacob);
%     
% end
% end
%     
%  for nn = 1:NN
%         fprintf("Jacobian for node %d is %.9f\n", nn,JL(nn,1));
%         
%  end


%     %------------------------  getNodalStress (1c)  ---------------------------
function [] = GetNodalStress();
    disp("Stresses at each node using method of average")
    ...
    for nn = 1:NN
        ...
        element_neigh = any(NOC==nn,2);
        all_elems = 1:NN;
        connected_elems = all_elems(element_neigh);
%          disp(connected_elems)
        % Find coordinate of node, and integration points for all elements
        % containing the current node
        node_coordinate = X(nn,:);
        
        integration_coordinates = zeros(length(connected_elems),NIP,2);
        stresses = STRL(connected_elems,:,:);
        ...
        for ii = 1:NIP
            ...
            for z = 1:length(connected_elems)
                  N = connected_elems(z);
                  X1 = X(NOC(N, 1), 1);
                  Y1 = X(NOC(N, 1), 2);
                  X2 = X(NOC(N, 2), 1);
                  ...
                  integration_coordinates(z,ii,1) = X1*N1+X2*N2+X3*N3+N4*X4;
                  integration_coordinates(z,ii,2) = Y1*N1+Y2*N2+Y3*N3+N4*Y4;                  
        ...
        integration_coordinates =reshape(integration_coordinates,[length(connected_elems)*NIP,2]);
        stresses = reshape(stresses, [],3);
%         disp(integration_coordinates)x
...
%         size(stresses)
        ...
        % Compute stresses about node using method of weighted averaging
        % sigma_xx
...
% %------------------------  getAvgStress (1d)  ---------------------------
 function [] = GetAvgStress();
    ...
    for ne=1:NE
        t_top(ne,:) = [SLE(ne,3) SLE(ne,2)];
        t_bottom(ne,:) = [-1*SLE(ne,3) -1*SLE(ne,2)];
        t_left(ne,:) = [-1*SLE(ne, 1) -1*SLE(ne,3)];
        t_right(ne,:) = [SLE(ne, 1) SLE(ne,3)];
    end
    ...
    disp("Traction difference between element and neighbor")
    disp("Element #,Neighbour Element #, traction_difference |t_i - t_j|");
       ...
       for el = 1:NE
          ...
          if el ==1
              nebor= [2;3];
              for kk=1:length(nebor)
              if nebor(kk)==2
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_top(el,:)-t_bottom(nebor(kk,:))));
              else
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_right(el,:)-t_left(nebor(kk,:))));
              end
              end
          elseif el == 2
              nebor = [1;4];
               for kk=1:length(nebor)
              if nebor(kk)==1
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_bottom(el,:)-t_top(nebor(kk,:))));
              else
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_right(el,:)-t_left(nebor(kk,:))));
              end
               end
          elseif el == 19
              nebor = [17;20];
               for kk=1:length(nebor)
              if nebor(kk)==17
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_left(el,:)-t_right(nebor(kk,:))));
              else
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_top(el,:)-t_bottom(nebor(kk,:))));
              end
               end
          ...
          elseif mod(el,2)==0
              nebor=[el-2;el-1;el+2];
              for kk=1:length(nebor)
               
              if nebor(kk)==(el-2)
             fprintf("%d %d %f\n",el,nebor(kk),norm(t_left(el,:)-t_right(nebor(kk,:))));
              elseif nebor(kk)==(el-1)
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_bottom(el,:)-t_top(nebor(kk,:))));
              else
              fprintf("%d %d %f\n",el,nebor(kk),norm(t_right(el,:)-t_left(nebor(kk,:))));  
